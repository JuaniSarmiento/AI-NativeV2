...
The system SHALL receive chat messages from the alumno and stream the tutor's response token-by-token.

#### Scenario: Alumno sends a message and receives streaming response
- **WHEN** the alumno sends `{ "type": "chat.message", "content": "...", "exercise_id": "UUID" }`
- **THEN** the server SHALL stream back multiple `{ "type": "chat.token", "content": "..." }` messages followed by `{ "type": "chat.done", "interaction_id": "UUID" }`
